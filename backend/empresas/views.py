...
class RegistroEmpresaView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        """
        Registra una nueva empresa con el admin autenticado como admin_id
        """
        ...
        try:
            ...
            # 1. Obtener el admin del sistema que está haciendo el registro
            ...
            # 2. Crear empresa con el admin que registra
            empresa = Empresa.objects.create(
                admin_id=admin_sistema,  # ← Admin que está registrando
...
class ListaEmpresasClienteAdminView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if not user.rol:
            logger.warning(f"Usuario {user.email} sin rol")
            return Empresa.objects.none()
        ...
        logger.warning(f"Usuario {user.email} con rol {user.rol.rol} intentó acceder a empresas")
        return Empresa.objects.none()
    ...
    def _obtener_estadisticas(self, queryset, user):
        ...
        # Info adicional para cliente
            ...
            try:
                ...
                # Contar total de empresas disponibles (todas las activas)
...
class DetalleEmpresaClienteAdminView(generics.RetrieveAPIView):
    ...
    def check_permissions(self, request):
        ...
        # Verificar que el usuario tenga rol admin o cliente
        if not user.rol or user.rol.rol not in ['admin', 'cliente']:
            self.permission_denied(
                request,
                message="Solo administradores o clientes pueden ver empresas",
